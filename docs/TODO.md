...
- [ ] **Add pagination to `list()` — only if the case count grows large.** Not needed now: `list()` is sub-10ms at the current scale (hundreds–low-thousands of cases). The blob store has no `LIMIT` — every `list()` pulls and `JSON.parse`s the whole (filtered) table, so cost scales with total rows, not page size. It starts to feel sluggish around ~50k rows and painful past ~100k. The fix when needed: `LIMIT`/keyset pagination with filtering+sorting pushed into SQL on the indexed generated columns (created_at/system_color/agrees), so only the returned page is parsed — this requires extending the `CaseStore` interface and the `/api/cases` handler. Note: this is the actual scaling lever, independent of the JSON-vs-normalized-tables choice.
