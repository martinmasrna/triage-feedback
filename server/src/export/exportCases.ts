...
function row(c: StoredCase): Record<(typeof CSV_COLUMNS)[number], string | number | undefined> {
  return {
    ...
    doctor_suggested_color: c.verdict?.suggested_color,
    verdict_changed: String(c.verdict_changed),
    decisive_rule: c.decision.decisive_rule?.name,
    fired_rules: c.decision.all_fired_rules.map((f) => f.name).join(";"),
