...
describe("default / no rules fired", () => {
  it("returns GREEN with no decisive rule for a well infant with normal vitals", () => {
    ...
    expect(r.all_fired_rules).toHaveLength(0);
    expect(r.decisive_rule).toBeNull();
...
describe("red-flag floor", () => {
  ...
  it("an unknown SpO2 does NOT fire the hypoxia rule", () => {
    ...
    expect(r.all_fired_rules.map((f) => f.name)).not.toContain("severe_hypoxia");
  });
  ...
  it("unresponsive discriminator forces RED regardless of normal vitals", () => {
    ...
    expect(r.color).toBe("RED");
    expect(r.decisive_rule?.name).toBe("unresponsive");
  });
  ...
  it("an 'absent' discriminator does not fire", () => {
    ...
    expect(r.all_fired_rules.map((f) => f.name)).not.toContain("unresponsive");
  });
  ...
  it("tie-break: among two RED rules the first in file order is decisive", () => {
    ...
    expect(r.decisive_rule?.name).toBe("unresponsive");
    expect(r.all_fired_rules.map((f) => f.name)).toEqual(expect.arrayContaining(["unresponsive", "severe_hypoxia"]));
  });
  ...
  it("shock = prolonged cap refill + tachycardia for age is RED", () => {
    ...
    expect(r.color).toBe("RED");
    expect(r.decisive_rule?.name).toBe("shock");
    // prolonged_cap_refill (ORANGE) and tachycardia_for_age (YELLOW) also fire, but RED wins
    expect(r.all_fired_rules.map((f) => f.name)).toEqual(expect.arrayContaining(["shock", "prolonged_cap_refill", "tachycardia_for_age"]));
  });
  ...
  it("hypotension below the band minimum is RED", () => {
    ...
    expect(low.decisive_rule?.name).toBe("hypotension_for_age");

    const ok = run({ age: { value: 3, unit: "years" }, vitals: { systolic_bp: 95 } });
    expect(ok.all_fired_rules.map((f) => f.name)).not.toContain("hypotension_for_age");
  });
});
...
describe("age-banded vitals", () => {
  it("HR just over the band upper is YELLOW (tachycardia)", () => {
    ...
    expect(r.decisive_rule?.name).toBe("tachycardia_for_age");
  });
  ...
  it("HR far over the band upper escalates to ORANGE (severe tachycardia), max-color wins", () => {
    ...
    expect(r.decisive_rule?.name).toBe("severe_tachycardia_for_age");
    expect(r.all_fired_rules.map((f) => f.name)).toEqual(expect.arrayContaining(["severe_tachycardia_for_age", "tachycardia_for_age"]));
  });
  ...
  it("the same HR is normal for a younger band (no rule fires)", () => {
    ...
    expect(r.all_fired_rules.map((f) => f.name)).not.toContain("tachycardia_for_age");
  });

  it("bradycardia for age is ORANGE", () => {
    const r = run({ age: { value: 6, unit: "months" }, vitals: { hr: 90 } }); // infant lower 110
    expect(r.color).toBe("ORANGE");
    expect(r.decisive_rule?.name).toBe("bradycardia_for_age");
  });
  ...
  it("diastolic BP below the band lower bound is YELLOW (diastolic hypotension)", () => {
    ...
    expect(r.color).toBe("YELLOW");
    expect(r.decisive_rule?.name).toBe("diastolic_hypotension_for_age");
  });
  ...
  it("diastolic BP above the band upper bound is YELLOW (diastolic hypertension)", () => {
    ...
    expect(r.color).toBe("YELLOW");
    expect(r.decisive_rule?.name).toBe("diastolic_hypertension_for_age");
  });
  ...
  it("diastolic BP within the band normal range fires neither rule", () => {
    ...
    expect(r.all_fired_rules.map((f) => f.name)).not.toEqual(
...
describe("age-conditioned rules", () => {
  it("fever in a young infant is ORANGE (outranks plain fever)", () => {
    ...
    expect(r.color).toBe("ORANGE");
    expect(r.decisive_rule?.name).toBe("fever_young_infant");
    expect(r.all_fired_rules.map((f) => f.name)).toEqual(expect.arrayContaining(["fever_young_infant", "fever"]));
  });

  it("the same fever in an older child is just YELLOW (fever)", () => {
    const r = run({ age: { value: 5, unit: "years" }, vitals: { temp: 38.5 } });
    expect(r.color).toBe("YELLOW");
    expect(r.decisive_rule?.name).toBe("fever");
    expect(r.all_fired_rules.map((f) => f.name)).not.toContain("fever_young_infant");
  });
});
